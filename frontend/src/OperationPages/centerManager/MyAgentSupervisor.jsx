...
export default function MyAgentSupervisor() {
  ...
  useEffect(() => {
    fetch(`http://localhost:4000/api/incrementalOperationPerAgentCenterManager`, {
      credentials: "include",
      headers: {
...
const filteredOperations = operations.filter((operation) => {
  ...
  return operation.nameAgent === selectedAgent && operationMonth === selectedMonth && operation.teamName === selectedTeam;
});

  return (
    <>
      <div className="titleOperationAndAgents">
        <h3>{`בחירת הציג מצטבר על פי חודש : ${selectedMonth}`}</h3>
<div>
      <FormControl
            variant="standard"
          sx={{ m: 1, minWidth: 120 }}
