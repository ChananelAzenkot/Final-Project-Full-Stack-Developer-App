...
useEffect(() => {
  fetch(
    `http://localhost:4000/api/operationId`,
    {
      credentials: "include",
      method: "GET",
      headers: {
    ...
    <>
      {
        <TableContainer component={Paper} id="container">
          <div className="btnGroup">
            {<CreateCards />}
          <Button variant="contained" id="BtnStart">הפסק נתונים</Button>
          </div>
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            ...
            <TableBody>
              {Array.isArray(operation) && operation.map(operation => (
                <StyledTableRow key={operation.id}>
                  ...
                  <StyledTableCell align="right">{operation.targets}</StyledTableCell>
                  <StyledTableCell align="right">
                    <IconButton>
                      <EditIcon />
                    </IconButton>
